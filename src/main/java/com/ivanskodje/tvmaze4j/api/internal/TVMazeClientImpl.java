...
public class TVMazeClientImpl implements TVMazeClient {
  ...
  public List<Show> showSearch(String query, boolean fetchEpisodes, boolean fetchSpecials) {
    ...
    ShowResultObject[] showResultObjects =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.SHOW_SEARCH, query), ShowResultObject[].class);
    if (showResultObjects != null) {
      Arrays.stream(showResultObjects)
          .map(TVMazeUtilities::getShowFromGsonObject)
          .forEach(
              show -> {
                if (fetchEpisodes) {
  ...
  public Show showSingleSearch(String query, boolean fetchEpisodes) {
    ...
    String formattedUrl =
        fetchEpisodes
            ? TVMazeEndpoints.SHOW_SINGLE_SEARCH_WITH_EPISODES
            : TVMazeEndpoints.SHOW_SINGLE_SEARCH;
    ShowObject showObject =
        REQUESTS.GET.makeRequest(String.format(formattedUrl, query), ShowObject.class);
    if (showObject == null) {
      return null;
    }
  ...
  public List<Person> peopleSearch(String query) {
    ...
    PersonResultObject[] personResultObjects =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.PEOPLE_SEARCH, query), PersonResultObject[].class);
    if (personResultObjects != null) {
      Arrays.stream(personResultObjects)
          .map(TVMazeUtilities::getPersonFromGsonObject)
          .forEach(
              person -> {
                people.add(person);
              });
  ...
  public List<Episode> showEpisodeList(int id, boolean fetchSpecials) {
    ...
    String apiUrl =
        (fetchSpecials) ? TVMazeEndpoints.EPISODE_LIST_WITH_SPECIALS : TVMazeEndpoints.EPISODE_LIST;
    EpisodeObject[] episodeObjects =
        REQUESTS.GET.makeRequest(String.format(apiUrl, id), EpisodeObject[].class);
    if (episodeObjects != null) {
      Arrays.stream(episodeObjects)
          .map(TVMazeUtilities::getEpisodeFromGsonObject)
  ...
  public Episode episodeByNumber(int showId, int season, int number) {
    EpisodeObject episodeObject =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.EPISODE_BY_NUMBER, showId, season, number),
            EpisodeObject.class);
  ...
  public List<Episode> episodesByDate(int showId, LocalDate date) {
    ...
    EpisodeObject[] episodeObjects =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.EPISODES_BY_DATE, showId, date), EpisodeObject[].class);
    if (episodeObjects != null) {
      Arrays.stream(episodeObjects)
          .map(TVMazeUtilities::getEpisodeFromGsonObject)
          .forEach(episodes::add);
  ...
  public List<Episode> episodesBySeason(int seasonId) {
    ...
    EpisodeObject[] episodeObjects =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.EPISODES_BY_SEASON, seasonId), EpisodeObject[].class);
    if (episodeObjects != null) {
      Arrays.stream(episodeObjects)
          .map(TVMazeUtilities::getEpisodeFromGsonObject)
          .forEach(episodes::add);
  ...
  public Show showInfo(int showId, boolean fetchCast) {
    ...
    ShowObject showObject =
        REQUESTS.GET.makeRequest(String.format(apiUrl, showId), ShowObject.class);
    if (showObject == null) {
      return null;
    }
    return TVMazeUtilities.getShowFromGsonObject(showObject);
  ...
  public Show showLookUpFromTvRage(int id) {
    ShowObject showObject =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.SHOW_LOOKUP_TVRAGE, id), ShowObject.class);
    if (showObject == null) {
      return null;
    }
    return TVMazeUtilities.getShowFromGsonObject(showObject);
  ...
  public Show showLookUpFromTheTvDb(int id) {
    ShowObject showObject =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.SHOW_LOOKUP_THETVDB, id), ShowObject.class);
    if (showObject == null) {
      return null;
    }
    return TVMazeUtilities.getShowFromGsonObject(showObject);
  ...
  public Show showLookUpFromImdb(String id) {
    ShowObject showObject =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.SHOW_LOOKUP_IMDB, id), ShowObject.class);
    if (showObject == null) {
      return null;
    }
    return TVMazeUtilities.getShowFromGsonObject(showObject);
  ...
  public List<Episode> getFullSchedule() {
    List<Episode> episodes = new CopyOnWriteArrayList<>();
    EpisodeObject[] episodeObjects =
        REQUESTS.GET.makeRequest(TVMazeEndpoints.SCHEDULE_FULL, EpisodeObject[].class);
    if (episodeObjects != null) {
      Arrays.stream(episodeObjects)
          .map(TVMazeUtilities::getEpisodeFromGsonObject)
          .forEach(episodes::add);
  ...
  public List<Season> getSeasons(int showId) {
    ...
    SeasonObject[] seasonObjects =
        REQUESTS.GET.makeRequest(
            String.format(TVMazeEndpoints.SHOW_SEASONS, showId), SeasonObject[].class);
    if (seasonObjects != null) {
      Arrays.stream(seasonObjects)
          .map(TVMazeUtilities::getSeasonFromGsonObject)
          .forEach(seasons::add);
    }
    return seasons;
  }
